...
const App: React.FC = () => {
    ...
    const handleProviderSelected = (provider: ScoredProvider) => {
        ...
        // Check if wallet is on Preprod network
        if (!wallet.isPreprodNetwork) {
            setError({
                type: 'wallet_required',
                message: 'Your wallet is not on PREPROD testnet. Please switch to Preprod testnet to use SkillForge.'
            });
            return;
        }
    ...
    // Step 7: Submit attestation using wallet functions
    const handleAttest = async (isLearner: boolean) => {
        if (!sessionId || !wallet.paymentAddress || !escrowTxId) return;

        setIsAttesting(true);
        try {
            // Get script UTXO from escrow transaction
            const scriptUTXO = `${escrowTxId}#0`; // Would be actual UTXO from escrow

            let txHash: string;
            if (isLearner) {
                // Use wallet attestLearner function
                txHash = await wallet.attestLearner({
                    sessionId,
                    scriptUTXO
                });
                setLearnerAttested(true);
            } else {
                // Use wallet attestMentor function
                txHash = await wallet.attestMentor({
                    sessionId,
                    scriptUTXO
                });
                setProviderAttested(true);
            }
    ...
    // Step 8-11: Mint NFT and display using wallet mintSessionNFT function
    const handleMintNFT = async (eventCardImage?: File) => {
        if (!sessionId || !wallet.isConnected || !intent || !intent.skill) {
            setError({
                type: null,
                message: 'Please ensure wallet is connected and session is active'
            });
            return;
        }
        ...
        try {
            console.log('[App] Minting NFT for session:', sessionId);

            // Use wallet mintSessionNFT function which handles signing and submission
            const txHash = await wallet.mintSessionNFT({
                sessionId,
                eventCardImage
            });

            console.log('[App] NFT minted successfully:', txHash);

            // Also get mint response for metadata
    ...
    return (
        <div className="app-shell">
            ...
            <section className="card mt-4">
                <WalletConnector />
                <WalletBalanceDisplay minAdaRequired={5} />
                                ...
                                <div className="flex-between">
                                    ...
                                    <div>
                                        <label
                                            ...
                                            className={`btn btn-ghost ${(!wallet.isConnected || !wallet.isPreprodNetwork) ? 'disabled' : ''}`}
                                            style={{ marginRight: '8px', pointerEvents: (!wallet.isConnected || !wallet.isPreprodNetwork) ? 'none' : 'auto', opacity: (!wallet.isConnected || !wallet.isPreprodNetwork) ? 0.5 : 1 }}
                                        >
                                        ...
                                        <button
                                            ...
                                            disabled={isMinting || !wallet.isConnected || !wallet.isPreprodNetwork}
                                        >
            ...
            <section className="card mt-4">
                ...
                <ProviderList
                    ...
                    disabled={!wallet.isPreprodNetwork && wallet.isConnected}
                />
            </section>
